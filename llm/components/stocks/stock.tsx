      ...
      <div className="text-green-400 rounded-2xl bg-zinc-950 pt-3 sm:pt-5">
        ...
        <div
          ...
          onPointerMove={(event) => {
            if (chartRef.current) {
              const { clientX } = event;
              const { left, right } = chartRef.current.getBoundingClientRect();
              let dateIdx = Math.round(clientX);
              dateIdx = dateIdx < 0 ? 0 : dateIdx;
              const time = getRange(right)[dateIdx];

              if (time) {
                setPriceAtTime({
                  time: format(time, "dd LLL yy"),
